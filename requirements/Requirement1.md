# getContainingRegistry

Chrome Canary now has support for Scoped Custom Registries
...
Define a module, 'getHighestCERNode' with a function of the same name that takes as a parameter a Node, and recursively does a parentElement | rootNode, checking if the customElementRegistry matches.  Stop at the rootNode and return the highest node with a customElementRegistry that matches the passed in element.
